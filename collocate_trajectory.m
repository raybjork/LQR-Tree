...
%   @output z0: initial guess for z; n_z by 1 vector
    ...
    problem.nonlcon = @(z) all_constraints(dynamics, z, N, nx, nu, dt);
...
function [c, ceq, dC, dCeq] = all_constraints(dynamics, z, N, nx, nu, dt)
    [ceq, dCeq] = dynamics_constraints(dynamics, z, N, nx, nu, dt);
    c = zeros(0,1);
    dC = zeros(0,numel(z));
    dC = sparse(dC)';
    dCeq = sparse(dCeq)';
...
%   @output dG: gradient of total accrued cost; nz by 1 vector
    ...
    for i=1:(N-1)
        ...
        g = g + ((z(u_i)^2) + (z(u_ip1)^2))*(dt / 2);
        %dG([u_i u_ip1]) = [dG(u_i) + z(u_i), dG(u_ip1) + z(u_ip1)];
        dG(u_i) = 2 * z(u_i) * dt;
    end
...
function [h,dH] = dynamics_constraints(dynamics, z, N, nx, nu, dt)
...
%   (N-1)*nx by nz matrix
    ...
    for i=1:(N-1)
        ...
        [h_i, dH_i] = dynamics_constraint_with_derivative(dynamics, ...
            z(x_i), z(u_i), z(x_ip1), z(u_ip1), dt);
        h(start: start + nx - 1) = h_i;
        dH(start: start + nx - 1, (i-1)*(nx+nu) + 1: (i+1)*(nx+nu)) = dH_i;
    end
